...
class Easom(Benchmark):
    ...
    def evaluate(self, x, *args):
        ...
        # Use tanh to smooth the exponential behavior and avoid overflow
        exp_term = np.tanh(-a) + 1  # This will give a smooth, bounded output
        # exp_term = np.exp(-a)

        # Calculate the result
        return -np.cos(x[0]) * np.cos(x[1]) * exp_term
